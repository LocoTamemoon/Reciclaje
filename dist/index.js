...
async function ensureHandoffSchema() {
    try {
        ...
        await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_old_ok BOOLEAN DEFAULT false");
        await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_new_ok BOOLEAN DEFAULT false");
        await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_cur_lat NUMERIC(9,6)");
        await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_cur_lon NUMERIC(9,6)");
        try {
            const c = await pool_1.pool.query("SELECT 1 FROM information_schema.table_constraints WHERE table_name='solicitudes' AND constraint_name='solicitudes_handoff_recolector_id_fkey' LIMIT 1");
            if (!c.rows[0]) {
                await pool_1.pool.query("ALTER TABLE solicitudes ADD CONSTRAINT solicitudes_handoff_recolector_id_fkey FOREIGN KEY (handoff_recolector_id) REFERENCES recolectores(id)");
            }
        }
        catch { }
    }
...
async function ensureActivosSchema() {
    try {
        await pool_1.pool.query("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS estado BOOLEAN DEFAULT true");
        await pool_1.pool.query("ALTER TABLE empresas ADD COLUMN IF NOT EXISTS estado BOOLEAN DEFAULT true");
        await pool_1.pool.query("ALTER TABLE resenas_empresas ADD COLUMN IF NOT EXISTS estado BOOLEAN DEFAULT true");
        await pool_1.pool.query("ALTER TABLE resenas_empresas_por_recolector ADD COLUMN IF NOT EXISTS estado BOOLEAN DEFAULT true");
        await pool_1.pool.query("ALTER TABLE resenas_usuarios ADD COLUMN IF NOT EXISTS estado BOOLEAN DEFAULT true");
        await pool_1.pool.query("ALTER TABLE resenas_usuarios_por_recolector ADD COLUMN IF NOT EXISTS estado BOOLEAN DEFAULT true");
        await pool_1.pool.query("ALTER TABLE resenas_recolectores ADD COLUMN IF NOT EXISTS estado BOOLEAN DEFAULT true");
    }
    catch { }
}
ensureActivosSchema();
async function ensureAdminSchema() {
    try {
        await pool_1.pool.query("CREATE TABLE IF NOT EXISTS admins (id SERIAL PRIMARY KEY, email TEXT NOT NULL UNIQUE, password_hash TEXT NOT NULL, nombre TEXT, apellidos TEXT, foto_perfil TEXT, estado BOOLEAN DEFAULT true, creado_en TIMESTAMPTZ DEFAULT NOW())");
        const email = process.env.ADMIN_EMAIL;
        const pass = process.env.ADMIN_PASSWORD;
        if (email && pass) {
            const bcrypt = require("bcryptjs");
            const hash = await bcrypt.hash(String(pass), 10);
            await pool_1.pool.query("INSERT INTO admins(email, password_hash, nombre, estado) VALUES($1,$2,$3,true) ON CONFLICT(email) DO UPDATE SET password_hash=EXCLUDED.password_hash", [String(email), String(hash), "Admin"]);
        }
    }
    catch { }
}
...
async function ensureEmpresasRucConstraint() {
    try {
        try {
            const c = await pool_1.pool.query("SELECT 1 FROM information_schema.table_constraints WHERE table_name='empresas' AND constraint_name='empresas_ruc_chk' LIMIT 1");
            if (!c.rows[0]) {
                await pool_1.pool.query("ALTER TABLE empresas ADD CONSTRAINT empresas_ruc_chk CHECK (ruc ~ '^[0-9]{11}$')");
            }
        }
        catch { }
    }
...
const handoffSimTimers = new Map();
const handoffSimProgress = new Map();
const handoffSimPoints = new Map();
const handoffSpeedMult = new Map();
global.__viajeSimHooks = {
    iniciar: async (sid) => {
    ...
    reanudar: async (sid) => {
        const arr = viajeSimPoints.get(sid) || [];
        if (!arr || arr.length < 2) {
            await global.__viajeSimHooks.iniciar(sid);
            return;
        }
...
app.get("/api/viajes/:sid/stream", (req, res) => {
    ...
    (async () => {
        try {
            const r = await pool_1.pool.query("SELECT handoff_state, handoff_cur_lat, handoff_cur_lon, handoff_pick_lat, handoff_pick_lon, handoff_recolector_id FROM solicitudes WHERE id=$1", [sid]);
            ...
            if (s && String(s.handoff_state || '') === 'en_intercambio') {
                ...
                if (hLat == null || hLon == null) {
                    try {
                        const rr = await pool_1.pool.query("SELECT lat, lon FROM recolectores WHERE id=$1", [Number(s.handoff_recolector_id || 0)]);
    ...
    try {
        ...
        if (!Number.isNaN(latNum) && !Number.isNaN(lonNum)) {
            (async () => {
                ...
                const pauseHandoff = Boolean(r.pauseAtHandoff);
...
app.post("/api/viajes/:sid/handoff/posicion", async (req, res) => {
    ...
    try {
        const sRes = await pool_1.pool.query("SELECT handoff_state, handoff_pick_lat, handoff_pick_lon FROM solicitudes WHERE id=$1", [sid]);
        const s = sRes.rows[0] || null;
        if (!s || String(s.handoff_state || '') !== 'en_intercambio') {
            res.status(422).json({ error: "no_handoff" });
            return;
        }
        if (!Number.isNaN(latNum) && !Number.isNaN(lonNum)) {
            try {
                await pool_1.pool.query("UPDATE solicitudes SET handoff_cur_lat=$2, handoff_cur_lon=$3 WHERE id=$1", [sid, latNum, lonNum]);
            }
            catch { }
            if (set) {
...
async function obtenerPuntosHandoff(sid) {
    const sRes = await pool_1.pool.query("SELECT handoff_pick_lat, handoff_pick_lon, handoff_recolector_id, handoff_cur_lat, handoff_cur_lon FROM solicitudes WHERE id=$1", [sid]);
    const s = sRes.rows[0] || null;
    if (!s)
        return [];
    ...
    if (r2Lat == null || r2Lon == null) {
        const r2 = await pool_1.pool.query("SELECT lat, lon FROM recolectores WHERE id=$1", [Number(s.handoff_recolector_id || 0)]);
        const rr = r2.rows[0] || null;
        r2Lat = rr?.lat != null ? Number(rr.lat) : null;
        r2Lon = rr?.lon != null ? Number(rr.lon) : null;
    }
    if (r2Lat == null || r2Lon == null || pickLat == null || pickLon == null)
        return [];
    try {
...
async function emitirHandoffSSE(sid, lat, lon) {
    const set = viajeStreams.get(sid);
    const r = await pool_1.pool.query("SELECT handoff_pick_lat, handoff_pick_lon FROM solicitudes WHERE id=$1", [sid]);
    const s = r.rows[0] || null;
    await pool_1.pool.query("UPDATE solicitudes SET handoff_cur_lat=$2, handoff_cur_lon=$3 WHERE id=$1", [sid, Number(lat), Number(lon)]);
    if (set) {
...
global.__handoffSimHooks = {
    iniciar: async (sid) => {
        ...
        if (i < pts.length) {
            const p0 = pts[i];
            await emitirHandoffSSE(sid, p0.lat, p0.lon);
        }
...
app.listen(env_1.env.port, () => {
    console.log(`Servidor iniciado en puerto ${env_1.env.port}`);
});
