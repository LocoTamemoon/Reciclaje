...
exports.recolectorRouter.get("/:id/en_curso", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_expires_at TIMESTAMPTZ");
    // cleanup expirados en_intercambio
    try {
        await pool_1.pool.query("UPDATE solicitudes SET handoff_state=NULL, handoff_recolector_id=NULL, handoff_old_ok=false, handoff_new_ok=false, estado='rumbo_a_empresa' WHERE handoff_state='en_intercambio' AND handoff_expires_at IS NOT NULL AND handoff_expires_at <= NOW()");
    }
    catch { }
    const rows = await pool_1.pool.query("SELECT * FROM solicitudes WHERE ((recolector_id=$1 AND tipo_entrega='delivery' AND estado_publicacion='aceptada_recolector' AND estado IN ('rumbo_usuario','cerca_usuario','rumbo_a_empresa','cerca_empresa','llego_empresa','entregado_empresa','empresa_confirmo_recepcion')) OR (handoff_state='en_intercambio' AND (handoff_expires_at IS NULL OR handoff_expires_at>NOW()) AND handoff_recolector_id=$1)) ORDER BY creado_en DESC LIMIT 1", [id]);
...
exports.recolectorRouter.get("/:id/perfil", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const r = await pool_1.pool.query("SELECT id, email, lat, lon, id_distrito, nombre, apellidos, dni, estado, reputacion_promedio, resenas_recibidas_count, trabajos_completados, foto_perfil, foto_documento, foto_vehiculo FROM recolectores WHERE id=$1", [id]);
    if (!r.rows[0]) {
        res.status(404).json({ error: "not_found" });
        return;
    }
...
exports.recolectorRouter.patch("/:id/perfil", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const id = Number(req.params.id);
    ...
    const vals = [];
    if (perfilPath) {
        fields.push("foto_perfil=$" + (vals.length + 2));
        vals.push(perfilPath);
    }
    if (docPath) {
        fields.push("foto_documento=$" + (vals.length + 2));
        vals.push(docPath);
    }
    if (vehPath) {
        fields.push("foto_vehiculo=$" + (vals.length + 2));
        vals.push(vehPath);
    }
    if (fields.length === 0) {
        const r = await pool_1.pool.query("SELECT id, email, lat, lon, id_distrito, nombre, apellidos, dni, estado, reputacion_promedio, resenas_recibidas_count, trabajos_completados, foto_perfil, foto_documento, foto_vehiculo FROM recolectores WHERE id=$1", [id]);
...
exports.recolectorRouter.post("/:id/ubicacion_actual", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        const actives = await pool_1.pool.query("SELECT id FROM solicitudes WHERE recolector_id=$1 AND tipo_entrega='delivery' AND estado_publicacion='aceptada_recolector' AND estado IN ('rumbo_usuario','cerca_usuario','rumbo_a_empresa','cerca_empresa','llego_empresa','entregado_empresa','empresa_confirmo_recepcion')", [id]);
...
exports.recolectorRouter.get("/handoff/publicados", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const r = await pool_1.pool.query("SELECT id, usuario_id, empresa_id, recolector_id, handoff_pick_lat, handoff_pick_lon, handoff_expires_at, foto_material_url FROM solicitudes WHERE handoff_state='publicado' AND (handoff_expires_at IS NULL OR handoff_expires_at > NOW()) ORDER BY creado_en DESC");
    const list = r.rows.filter((row) => Number(row.recolector_id || 0) !== (Number.isNaN(viewerId) ? 0 : viewerId));
    res.json(list);
}));
exports.recolectorRouter.post("/:sid/handoff/request", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const sid = Number(req.params.sid);
    const recolectorId = Number(req.body?.recolector_id);
    if (!sid || Number.isNaN(recolectorId)) {
        res.status(400).json({ error: "invalid_body" });
        return;
    }
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_state TEXT");
    ...
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_expires_at TIMESTAMPTZ");
    const sRes = await pool_1.pool.query("SELECT estado, recolector_id FROM solicitudes WHERE id=$1", [sid]);
    const s = sRes.rows[0] || null;
    if (!s || Number(s.recolector_id) !== recolectorId) {
        res.status(403).json({ error: "no_owner" });
        return;
    }
    const est = String(s.estado || '');
    if (!(est === 'rumbo_a_empresa' || est === 'cerca_empresa')) {
        res.status(422).json({ error: "etapa_invalida" });
        return;
    }
    const rpos = await pool_1.pool.query("SELECT lat, lon FROM recolectores WHERE id=$1", [recolectorId]);
    ...
    await pool_1.pool.query("UPDATE solicitudes SET handoff_state='publicado', handoff_pick_lat=$2, handoff_pick_lon=$3, handoff_expires_at = NOW() + INTERVAL '15 minutes' WHERE id=$1", [sid, pickLat, pickLon]);
    const subs = global.__notifSubs || (global.__notifSubs = {});
    const notify = async (destRole, destId, tipo, mensaje) => {
        const safeId = (destId != null && !Number.isNaN(destId) && destId > 0) ? destId : null;
        if (!safeId)
...
exports.recolectorRouter.post("/:sid/handoff/accept", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const sid = Number(req.params.sid);
    const newRecoId = Number(req.body?.recolector_id);
    if (!sid || Number.isNaN(newRecoId)) {
        res.status(400).json({ error: "invalid_body" });
        return;
    }
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_state TEXT");
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_recolector_id INTEGER");
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_expires_at TIMESTAMPTZ");
    const sRes = await pool_1.pool.query("SELECT estado, recolector_id, handoff_state, handoff_expires_at FROM solicitudes WHERE id=$1", [sid]);
    const s = sRes.rows[0] || null;
    if (!s) {
        res.status(404).json({ error: "not_found" });
        return;
    }
    if (String(s.handoff_state || '') !== 'publicado') {
        res.status(422).json({ error: "no_publicado" });
        return;
    }
    const exp = s.handoff_expires_at ? new Date(s.handoff_expires_at) : null;
    if (exp && exp.getTime() <= Date.now()) {
        await pool_1.pool.query("UPDATE solicitudes SET handoff_state=NULL, handoff_expires_at=NULL WHERE id=$1", [sid]);
        res.status(422).json({ error: "expirado" });
        return;
    }
    if (Number(s.recolector_id) === newRecoId) {
        res.status(422).json({ error: "mismo_recolector" });
        return;
    }
    ...
    if (busy.rows[0]) {
        res.status(422).json({ error: "recolector_ocupado" });
        return;
    }
    await pool_1.pool.query("UPDATE solicitudes SET handoff_state='en_intercambio', handoff_recolector_id=$2, handoff_old_ok=false, handoff_new_ok=false, handoff_expires_at = NOW() + INTERVAL '5 minutes' WHERE id=$1", [sid, newRecoId]);
...
exports.recolectorRouter.post("/:sid/handoff/confirm_old", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const sid = Number(req.params.sid);
    const recoId = Number(req.body?.recolector_id);
    if (!sid || Number.isNaN(recoId)) {
        res.status(400).json({ error: "invalid_body" });
        return;
    }
    ...
    if (String(s.handoff_state || '') !== 'en_intercambio') {
        res.status(422).json({ error: "no_intercambio" });
        return;
    }
...
exports.recolectorRouter.post("/:sid/handoff/confirm_new", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_pick_lon NUMERIC(9,6)");
    await pool_1.pool.query("ALTER TABLE solicitudes ADD COLUMN IF NOT EXISTS handoff_recolector_id INTEGER");
    const sRes = await pool_1.pool.query("SELECT usuario_id, empresa_id, recolector_id, handoff_state, handoff_recolector_id, handoff_old_ok, handoff_pick_lat, handoff_pick_lon FROM solicitudes WHERE id=$1", [sid]);
    const s = sRes.rows[0] || null;
    if (!s || Number(s.handoff_recolector_id) !== recoId) {
        res.status(403).json({ error: "no_new_owner" });
        return;
    }
    if (String(s.handoff_state || '') !== 'en_intercambio') {
        res.status(422).json({ error: "no_intercambio" });
    ...
    await pool_1.pool.query("UPDATE solicitudes SET handoff_new_ok=true WHERE id=$1", [sid]);
...
exports.recolectorRouter.get("/trabajos/:sid/detalle", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const pickRecoIdRes = await pool_1.pool.query("SELECT handoff_recolector_id FROM solicitudes WHERE id=$1", [sid]);
    const handRecoId = pickRecoIdRes.rows[0]?.handoff_recolector_id != null ? Number(pickRecoIdRes.rows[0].handoff_recolector_id) : null;
    const currentRecoId = Number(s.recolector_id);
    ...
    try {
        if (viewerId != null && !Number.isNaN(viewerId)) {
            const chkE = await pool_1.pool.query("SELECT 1 FROM resenas_empresas_por_recolector WHERE empresa_id=$1 AND recolector_id=$2 AND transaccion_id=$3 LIMIT 1", [Number(s.empresa_id), viewerId, Number(tx.id)]);
            ya_emp_por_reco = (chkE.rowCount || 0) > 0;
            const chkU = await pool_1.pool.query("SELECT 1 FROM resenas_usuarios_por_recolector WHERE usuario_id=$1 AND recolector_id=$2 AND transaccion_id=$3 LIMIT 1", [Number(s.usuario_id), viewerId, Number(tx.id)]);
