...
recolectorRouter.post("/:sid/aceptar", asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const st = await pool.query("SELECT estado FROM recolectores WHERE id=$1", [Number(recolector_id)]);
    const activo = Boolean(st.rows[0]?.estado);
    if (!activo) { res.status(422).json({ error: "recolector_inactivo" }); return; }
  } catch {}
  let s: any = null;
  try {
...
recolectorRouter.post("/:sid/handoff/confirm_old", asyncHandler(async (req: Request, res: Response) => {
  ...
  const okOld = true;
  if (okOld && okNew) {
    const curRes = await pool.query("SELECT handoff_cur_lat, handoff_cur_lon, handoff_pick_lat, handoff_pick_lon FROM solicitudes WHERE id=$1", [sid]);
    ...
    const dPick = (!isNaN(r2Lat) && !isNaN(r2Lon) && !isNaN(pickLat) && !isNaN(pickLon)) ? haversineKm(r2Lat, r2Lon, pickLat, pickLon) : Infinity;
    if (dPick <= 0.05) {
